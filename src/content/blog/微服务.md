...
**微前端方案的核心**是“主从”思想，即包括一个**基座**（`MainApp`）应用和若干个**微（`MicroApp`）应用**，基座应用大多数是一个前端`SPA`项目（单页面应用），主要负责应用注册，路由映射，消息下发等，而微应用是独立前端项目，这些项目不限于采用`React，Vue，Angular`或者`JQuery`开发，每个微应用注册到基座应用中，由基座进行管理，但是如果脱离基座也是可以单独访问
