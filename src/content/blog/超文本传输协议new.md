...
2. **改进的拥塞控制。**
   ...
   TCP 使用ACK（确认），还有基于字节序号的 `Sequence Number`为了保证可靠性，而QUIC也有使用ACK（确认）并且以 `Packet Number` 代替了 TCP 的 `sequence number`；
   ...
   - 当`Packet Number` 发生了丢失，`Packet Number` 传递的数值（也就是那个Number）会不一样，因为他是按照严格递增的，此时就可以发现传送的失误了。（在RTO 发生后，可以精确判断Ack 数据是原始请求的响应，还是重传请求的响应）
